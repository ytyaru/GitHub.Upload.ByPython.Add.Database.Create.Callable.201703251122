...
#    def Run(self, path_dir_pj, user_name=None, description=None, homepage=None):
        ...
        path_config = os.path.join(path_dir_this, 'config.ini')
        print(path_config)
        config.read(path_config)
        print(config['Path']['DB'])
        if os.path.isabs(config['Path']['DB']):
            path_db = config['Path']['DB']
        else:
            path_db = os.path.join(path_dir_this, config['Path']['DB'])
        if not(os.path.isdir(path_db)):
            raise Exception('config.iniのPath.DBにはファイルではなくディレクトリのパスを指定してください。')
        print(path_db)
